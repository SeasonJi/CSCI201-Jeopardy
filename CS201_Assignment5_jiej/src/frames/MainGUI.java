...
public class MainGUI extends JFrame {
	...
	//hw5 new components TODO
	public ArrayList<JPanel> clockPanels;
	public JLabel[] clockLabels;
	...
	private JPanel createProgressPanel() {
		//hw5 new components
		clockPanels = new ArrayList<JPanel>();
		for(int i = 0; i < gameData.getNumberOfTeams(); i++)
		{
			JPanel clockPanelTemp = new JPanel();
			clockPanels.add(clockPanelTemp);
			clockPanels.get(i).setBackground(Color.darkGray);
		}		
		...
		// setting appearances
		AppearanceSettings.setBackground(AppearanceConstants.lightBlue,usernameLabel, southEastPanel, updatesLabel,updatePanel, updatesScrollPane,
				updatesTextArea);
		...
		usernameLabel.setFont(AppearanceConstants.fontSmall);
		pointsPanel.setBackground(Color.darkGray);
		updatePanel.setBorder(BorderFactory.createLineBorder(Color.darkGray));
		...
		southEastPanel.add(updatePanel, BorderLayout.NORTH);
		...
		for (int i = 0; i < gameData.getNumberOfTeams(); i++) {
			...
			pointsPanel.add(clockPanels.get(i));
